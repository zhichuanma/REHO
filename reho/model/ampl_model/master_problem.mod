...
var lca_operation{k in Lca_kpi} default 0;
...
lca_operation[k] = sum{l in ResourceBalances, u in Units inter UnitsOfLayer[l]} lca_op[k, u] + sum{h in House} lca_house_op[k, h];
...
lca_tot[k] = lca_constr[k] + sum{l in ResourceBalances} lca_res[k, l] + lca_operation[k];
